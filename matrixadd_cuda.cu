...
__global__ void gpuMatAdd(float *A, float *B, float *C, int N, int M) 
{
  ...
  if (i >= N) printf("index i is greater than N\n");
  if (j >= M) printf("index j is greater than M\n");
...
int main(int argc, char *argv[]) {
  ...
  cudaMalloc((void **)&A_dev, N * M * sizeof(float));
  cudaMalloc((void **)&B_dev, N * M * sizeof(float));
  cudaMalloc((void **)&C_dev, N * M * sizeof(float));
  //----------------------------
  ...
  gpuMatAdd<<<block, grid>>>(A, B, C, N, M)
  //----------------------------

  //----- insert CUDA code -----
  // copy data from device to host
  cudaMemcpy(C, C_dev, sizeof(C_dev), cudaMemcpyDeviceToHost);
  //----------------------------
